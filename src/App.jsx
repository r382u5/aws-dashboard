...
- 問題文の型: 複雑なシナリオや架空の企業名は一切使用しないでください。「〜〜を提供するAWSサービスはどれですか？」「〜〜という概念を最も適切に説明しているものはどれですか？」といった、シンプルで直接的な疑問文にしてください。
- 選択肢の型: 単一のサービス名（例：Amazon S3）、または短い専門用語（例：スケーラビリティ）のみで構成してください。
- 不正解の条件: 全く関係ないサービスではなく、初心者や未経験者が混同しやすい類似サービス（例：S3とEBS、EC2とLambda、IAMとCognitoなど）を配置してください。`;
            } else if (difficulty === '中級') {
                difficultyInstruction = `【難易度：中級 の作成ルール】
目的：AWS認定クラウドプラクティショナーの「本番試験の標準レベル」を完全に再現する。
- 問題文の型: 必ず「ある企業が〜」「あるスタートアップが〜」から始まる架空のユースケース（シナリオ）を作成してください。
- 要件の提示: 問題文の中には、解決すべき課題や要件（例：「初期費用を抑えたい」「ミリ秒単位の応答が必要」「リレーショナルデータベースを使いたい」など）を明確に1〜2つ提示してください。
- 選択肢の構成: 単一のサービス名、または短い説明（例：「リザーブドインスタンスを購入する」）としてください。
- 巧妙な不正解（最重要）: 選択肢の中には、「問題文の要件の一部は満たすが、別の要件（例：コスト、データ構造、運用負荷など）を満たさないために不正解となる実在のAWSサービス」を必ず含め、受験者が深く考えないと正解できないようにしてください。`;
            } else {
...
4. 解説のフォーマット: 正解の理由だけでなく、必ず「他のすべての不正解の選択肢がなぜ間違っているのか」の具体的な理由を含めて詳しく記載すること。
