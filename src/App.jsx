...
- 問題文の制約: 必ず「ある企業が〜」「ある組織が〜」から始まるシナリオ問題にすること。「コストを下げたい」などの「要件・課題は1つだけ」に絞ること。過剰な企業設定は不要ですが、課題を説明する短い導入は残し、全体を2〜4行程度でまとめること。
- 出題パターンの制約: 以下の2つのパターンのいずれかで作成すること。パターンAとパターンBの出力割合が半々になるように意識すること。
  パターンA: 「ある企業が〇〇という課題を抱えています。この要件を満たすために最適なAWSサービスはどれですか？」(シナリオからサービス名を問う)
  パターンB: 「ある企業が〇〇という課題を解決するために [特定のAWSサービス名] の導入を検討しています。このサービスを導入することで得られるメリット（または機能）として最も適切なものはどれですか？」(シナリオとサービス名から、機能の文章を問う)
...
- 問題文の制約: 必ず「ある企業が〜」「ある組織が〜」から始まるシナリオ問題にすること。「高可用性を維持しつつ、コストを最小限に抑えたい」などの「要件・課題を2つ以上」設定すること。過剰な企業設定は不要ですが、課題を説明する短い導入は残し、全体を2〜4行程度でまとめること。
- 出題パターンの制約: 以下の2つのパターンのいずれかで作成すること。パターンAとパターンBの出力割合が半々になるように意識すること。
  パターンA: 「ある企業が〇〇と△△という課題を抱えています。この要件を満たすために最適なAWSサービス（またはアクション）はどれですか？」
  パターンB: 「ある企業が〇〇と△△という課題を解決するために [特定のAWSサービス名] の導入を検討しています。このサービスが要件を満たせる理由として最も適切なものはどれですか？」
...
5. 問題文の簡潔さ: 問題文は本番試験を模倣し、簡潔にすること。解答に全く関係のない長すぎる背景（架空の企業の業種、規模、歴史など）は不要ですが、シナリオ問題（中級・上級）においては、問題の状況を理解するための必要最低限の文脈（ある企業が〇〇をしようとしている、等）は必ず含めること。
