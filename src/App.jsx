...
- 上級（本番難問）: 複数の要件（コスト、管理の手間など）や、Well-Architected Frameworkなどのアーキテクチャの原則が絡むシナリオ問題。
  ※上級の選択肢は単一のサービス名だけでなく、「Amazon CloudFront と Amazon S3」のような【複数のサービスの組み合わせ】や、「AWS Organizations で SCP を適用する」のような【具体的なアーキテクチャやアクションの記述】を必ず含めること。

【選択肢（ディストラクター）の作成ルール】
- 架空のサービス名は絶対に作成せず、すべて実在するAWSの公式サービスを使用すること。
- 不正解の選択肢はランダムなサービスではなく、「同じカテゴリだが、問題文の特定の要件（例：データベースの種類、リージョンかグローバルか）を満たさないため不正解となるサービス」を必ず含めて、受験者を迷わせる巧妙な構成にすること。
